...
public:
   ...
   void scene()
   {
      ...
      // Question 1 - B2
      vec3 d20 = vec3(4,0,2);
      quat R20 = glm::angleAxis((float) M_PI/4.0f, vec3(0,0,1));
      ...
      // Question 2
      Transform F21 = F20.inverse() * F10; // transform from frame 2 to frame 1
      vec3 value = F21.transformPoint(vec3(0));
      if (!once) std::cout << "Position of b2 relative to b1 is " << value << std::endl;

      // Question 3
      Transform F32 = F30.inverse() * F20; // transform from frame 3 to frame 2
      ...
      Transform up(R10, vec3(0,2,0));
      Transform F = F10 * up * F20.inverse();
