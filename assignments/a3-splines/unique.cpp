...
void CurveEditor::scene() {
  ...
  if(mSpline.getInterpolationType() == "Linear"){
    for(int i=0; i<mSpline.getNumKeys()-1; i++){
      glm::vec3 segmentColor = mSpline.getColor(i);
    ...
    for(int k=0; k<mSpline.getNumSegments(); k++){
      glm::vec3 segmentColor = mSpline.getColor(k);
...
void CurveEditor::addPoint(const vec3& p) {
  ...
  mSpline.appendKey(mSpline.getNumKeys(), p, drawColor);
}
...
void CurveEditor::listControls() {
  ...
  std::cout << "r: adjust line redness" << std::endl;
  std::cout << "g: adjust line greenness" << std::endl;
  std::cout << "b: adjust line blueness" << std::endl;
...
void CurveEditor::keyUp(int pKey, int mods) {
  if (pKey == 'p') {
    ...
    float redness = drawColor[0] + 0.1;
    if(redness > 1){
      redness -= 1;
    }
    drawColor[0] = redness;
    std::cout << "Drawing color: " << drawColor << std::endl;
  } else if (pKey == 'G') {
    float greenness = drawColor[1] + 0.1;
    if(greenness > 1){
      greenness -= 1;
    }
    drawColor[1] = greenness;
    std::cout << "Drawing color: " << drawColor << std::endl;
  } else if (pKey == 'B') {
    float blueness = drawColor[2] + 0.1;
    if(blueness > 1){
      blueness -= 1;
    }
    drawColor[2] = blueness;
    std::cout << "Drawing color: " << drawColor << std::endl;
  }
}
